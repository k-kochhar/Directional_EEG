...
# Hyperparameters
lr = 5e-4
epochs = 10 if TESTING else 100  # Use fewer epochs when testing
batch_size = 64
num_class = 10  # 10 directions for detailed angle classification
...
def create_model(sample_len=128, channels_num=64, lr=5e-4):
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dense(num_class, activation='softmax')  # Output 10 classes for angles
    ])
...
def preprocess_data(sub_id, freq_bands=[[1, 50]], data_folder="DATA", fs_new=128, label_type='all_direction'):
    ...
    Returns:
        ...
        label_list: List of corresponding labels for 10-direction classification
    """
    # Define detailed direction labels (10 classes):
    # 0 = 90° (left), 1 = 90° (right)
    # 2 = 60° (left), 3 = 60° (right)
    # 4 = 45° (left), 5 = 45° (right)
    # 6 = 30° (left), 7 = 30° (right)
    # 8 = 5° (left), 9 = 5° (right)
    direction_labels = [0, 1, 1, 0, 2, 3, 2, 3, 4, 5, 5, 4, 6, 7, 6, 7, 8, 9, 9, 8]
    ...
    for j in selected_trails:
        ...
        try:
            ...
            # Assign the label based on the trial index using the 10-class labeling
            label = direction_labels[j]
...
def main():
    ...
    for 10-direction classification. It then trains a model on the training folds and evaluates it on the
    test folds, collecting results.
    """
...
if __name__ == '__main__':
    ...
    Processes the data, trains models for 10-direction classification, and saves results.
    """
    print("Starting 10-direction EEG classification...")
    ...
    model_name = "CNN_10dir"  # Using CNN model for 10-direction classification
    ...
    with open(result_file_path, 'w') as file:
        # Write header with direction class explanations
        file.write("# 10-Direction Classification Results\n")
        file.write("# Direction Classes:\n")
        file.write("# 0 = 90° (left), 1 = 90° (right)\n")
        file.write("# 2 = 60° (left), 3 = 60° (right)\n")
        file.write("# 4 = 45° (left), 5 = 45° (right)\n")
        file.write("# 6 = 30° (left), 7 = 30° (right)\n")
        file.write("# 8 = 5° (left), 9 = 5° (right)\n\n")
    ...
    lines_to_save = [
        "# 10-Direction Classification - Average Results",
        "# Direction Classes:",
        "# 0 = 90° (left), 1 = 90° (right)",
        "# 2 = 60° (left), 3 = 60° (right)",
        "# 4 = 45° (left), 5 = 45° (right)",
